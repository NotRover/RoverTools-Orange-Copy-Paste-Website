...
import QuickPastePopup from '../../../components/landing/mocks/QuickPastePopup.astro';
import CapturePopup from '../../../components/landing/mocks/CapturePopup.astro';
...
Each row has a slot number. Press <kbd>1</kbd> through <kbd>9</kbd> (then <kbd>0</kbd>) and that entry is written to the clipboard and pasted into the app you came from. You can also arrow through the list and press <kbd>Enter</kbd>, or click a row. Hold <kbd>Alt</kbd> with the number (or <kbd>Alt</kbd>+<kbd>Enter</kbd>) to paste and drop the entry from Recent in one step. Each row also has inline Pin, Copy and Delete buttons on hover.
...
It dismisses on <kbd>Esc</kbd>, on losing focus, or with the close button.
...
- Drag either popup by its header to move it where you want.
